...
ADMIN_ID = 1678146043          
OTZIV_KANAL_ID = "@coinsotziv" 

bot = Bot(token=BOT_TOKEN)
dp = Dispatcher()
router = Router()

# Ma'lumotlar o'chib ketmasligi uchun Amvera /data doimiy xotirasi
...
📱 <i>Android tizimi orqali akkauntga kirib yuklanadi!</i>"""
    ...
    await bot.send_photo(chat_id=ADMIN_ID, photo=receipt_file_id, caption=admin_text, reply_markup=admin_kb, parse_mode="HTML")
...
async def show_setka(callback: CallbackQuery):
    ...
    if not matches:
        ...
        await callback.answer()
        return
    text = "📊 <b>JORIY TURNIR SETKASI (JUFTLIKLAR)</b> 📊\n\n"
...
async def show_rating(callback: CallbackQuery):
    ...
    if not players:
        ...
        await callback.answer()
        return
    text = "🏆 <b>EFOOTBALL REYTING JADVALI</b> 🏆\n\n⏱ O' | G' | D | M | <b>OCHKO</b> | O'yinchi\n----------------------------------------\n"
    for index, player in enumerate(players, start=1):
        username, played, wins, draws, losses, points = player
        medal = "🥇" if index == 1 else "🥈" if index == 2 else "🥉" if index == 3 else f"{index}."
        text += f"{medal} {played} | {wins} | {draws} | {losses} | <b>{points}</b> | @{username}\n"
    await callback.message.answer(text, parse_mode="HTML")
    await callback.answer()
...
async def admin_start_qura(message: Message):
    ...
    matches_to_save = []
    text = "🎲 <b>AVTOMATIK QUR'A NATIJALARI (SETKA):</b>\n\n"
...
async def admin_change_score(message: Message):
    try:
        ...
        update_player_score(user_id, username, result)
        await message.answer(f"✅ @{username} uchun ochko yangilandi: {result.upper()}")
    except:
        await message.answer("❌ Xato format. To'g'ri format: `/score [user_id] [win/draw/loss]`")

@router.message(Command("clear_turnir"), F.from_user.id == ADMIN_ID)
async def admin_clear_tournament(message: Message):
    reset_tournament()
    await message.answer("🗑 Barcha turnir ishtirokchilari tozalandi!")

# ============================================================
# LOYIHANI ISHGA TUSHIRISH (MAIN)
# ============================================================
async def main():
    init_all_dbs()
    dp.include_router(router)
    print("Bot Amvera serverida muvaffaqiyatli ishlamoqda...")
